...
(async () => {
  ...
  let max24hrs = data.high_24h.usd.toLocaleString("pt-BR", {
    style: "currency",
    currency: "USD",
  });
  let min24hrs = data.low_24h.usd.toLocaleString("pt-BR", {
    style: "currency",
    currency: "USD",
  });
  let priceChange7d = data.price_change_percentage_7d.toFixed(2);
  let priceChange7dString = `7 dias: ${
    priceChange7d > 0 ? "+" : ""
  }${priceChange7d}%`;
  let priceChange30d = data.price_change_percentage_30d.toFixed(2);
  let priceChange30dString = `30 dias: ${
    priceChange30d > 0 ? "+" : ""
  }${priceChange30d}%`;
  let priceChange60d = data.price_change_percentage_60d.toFixed(2);
  let priceChange60dString = `60 dias: ${
    priceChange60d > 0 ? "+" : ""
  }${priceChange60d}%`;
  let priceChange1y = data.price_change_percentage_1y.toFixed(2);
  let priceChange1yString = `1 ano: ${
    priceChange1y > 0 ? "+" : ""
  }${priceChange1y}%`;
  let marketcap = data.market_cap.usd.toLocaleString("pt-BR", {
    style: "currency",
    currency: "USD",
  });
  let marketcapString = `total: ${marketcap}`;
  let marketcapChange = data.market_cap_change_percentage_24h.toFixed(2);
  let marketcapChangeString = `24hrs: ${
    marketcapChange > 0 ? "+" : ""
  }${marketcapChange}%`;
  let supplyString = `${data.circulating_supply.toLocaleString(
    "pt-BR"
